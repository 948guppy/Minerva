...
class Pages:
    ...
    async def numbered_page(self):
        """任意のページに移動します"""
        to_delete = []
        to_delete.append(await self.channel.send('表示したいヘルプのページ番号を送信してください'))
        ...
        try:
            ...
            if page != 0 and page <= self.maximum_pages:
                ...
                to_delete.append(await self.channel.send(f'存在しないページを表示しようとしています ({page}/{self.maximum_pages})'))
                await asyncio.sleep(5)
    ...
    async def show_help(self):
        ...
        embed.clear_fields()
        embed.description = '\n'.join(messages)
        embed.set_footer(text=f'このヘルプの前は {self.current_page} ページ目を参照していました')
