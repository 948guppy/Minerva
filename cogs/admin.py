...
class Admin(commands.Cog):
    ...
    def find_modules_from_git(self, output):
        ...
        for file in files:
            ...
            if root.startswith('cogs/'):
                # A submodule is a directory inside the main cogs directory for
                # my purposes
    ...
    async def repl(self, ctx):
        ...
        if ctx.channel.id in self.sessions:
            await ctx.send('このチャンネルでは既にREPLセッションになっています。`exit()` または `quit` でこのセッションを終了できます。')
            return

        self.sessions.add(ctx.channel.id)
        await ctx.send('実行したいコードを入力してください。 `exit()` または `quit` でこのモードを終了します。')
        ...
        while True:
            try:
                ...
                await ctx.send('REPLセッションから退出します。')
            ...
            try:
                if fmt is not None:
                    if len(fmt) > 2000:
                        await ctx.send('メッセージが長すぎるため送信できませんでした。')
                    else:
                        await ctx.send(fmt)
